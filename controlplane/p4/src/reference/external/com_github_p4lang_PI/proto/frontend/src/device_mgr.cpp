...
class DeviceMgrImp {
 public:
  ...
  Status write(const p4v1::WriteRequest &request) {
    ...
    // BEGIN
    std::cout << "DeviceMgr::write (2) -> before DeviceMgr::write_" << std::endl;
    // END
  ...
  Status table_write(p4v1::Update::Type update,
                     ...
                     SessionTemp *session) {
    // BEGIN
    std::cout << "DeviceMgr::table_write -> starting. About to check P4 ID is correct = " 
      << table_entry.table_id() << std::endl;
    // END
    if (!check_p4_id(table_entry.table_id(), P4Ids::TABLE))
      return make_invalid_p4_id_status();

    // BEGIN
    std::cout << "DeviceMgr::table_write -> starting. P4 ID is correct = " 
      << table_entry.table_id() << std::endl;
    // END

    if (table_entry.has_time_since_last_hit()) {
      // BEGIN
      std::cout << "DeviceMgr::table_write -> error with time since last hit" << std::endl;
      // END
      RETURN_ERROR_STATUS(
          Code::INVALID_ARGUMENT,
    ...
    // BEGIN
    std::cout << "DeviceMgr::table_write -> update = " << update << std::endl;
    // END
 ...
 private:
  ...
  Status write_(const p4v1::WriteRequest &request) {
    ...
    // BEGIN
    std::cout << "DeviceMgr::write_ -> starting method" << std::endl;
    // END
    Status status;
    status.set_code(Code::OK);
    ...
    for (const auto &update : request.updates()) {
      ...
      // BEGIN
      std::cout << "DeviceMgr::write_ -> entity = " << entity.SerializeAsString() << std::endl;
      std::cout << "DeviceMgr::write_ -> entity case = " << entity.entity_case() << std::endl;
      // END
      switch (entity.entity_case()) {
        ...
        case p4v1::Entity::kTableEntry:
          // BEGIN
          std::cout << "DeviceMgr::write_ -> table_write . update type = " << update.type() << std::endl;
          std::cout << "DeviceMgr::write_ -> table_write . entity table entry = " << entity.table_entry().SerializeAsString() << std::endl;
          // END
  ...
  Status construct_action_data(uint32_t table_id, const p4v1::Action &action,
    ...
    // BEGIN
    std::cout << "DeviceMgr::construct_action_data -> starting method" << std::endl;
    std::cout << "DeviceMgr::construct_action_data -> action type = " 
      << action.GetTypeName() << std::endl;
    // END
    (void) table_id;
    RETURN_IF_ERROR(validate_action_data(p4info.get(), action));
    // BEGIN
    std::cout << "DeviceMgr::construct_action_data -> properly validated action data" << std::endl;
    // END
    action_entry->init_action_data(p4info.get(), action.action_id());
    // BEGIN
    std::cout << "DeviceMgr::construct_action_data -> properly initialised action data" << std::endl;
    // END
    auto action_data = action_entry->mutable_action_data();
    // BEGIN
    std::cout << "DeviceMgr::construct_action_data -> action_data = " << action_data << std::endl;
    // END
    for (const auto &p : action.params()) {
      // BEGIN
      std::cout << "DeviceMgr::construct_action_data -> adding param with id = " 
        << p.param_id() << " and data = " << p.value().data() << " and size = "
  ...
  Status construct_action_entry(uint32_t table_id,
                                ...
                                pi::ActionEntry *action_entry) {
    // BEGIN
    std::cout << "DeviceMgr::construct_action_entry -> starting method" << std::endl;
    std::cout << "DeviceMgr::construct_action_entry -> table_action.type_case() = " << 
      table_action.type_case() << std::endl;
    // END
    switch (table_action.type_case()) {
  ...
  Status table_insert(const p4v1::TableEntry &table_entry,
                      SessionTemp *session) {
    // BEGIN
    std::cout << "DeviceMgr::table_insert -> starting table_insert = " << std::endl;
    // END
    const auto table_id = table_entry.table_id();
    // BEGIN
    std::cout << "DeviceMgr::table_insert -> table_entry.is_default_action() = "
      << table_entry.is_default_action() << std::endl;
    // END
    ...
    // BEGIN
    std::cout << "DeviceMgr::table_insert -> table_entry.has_action() = "
      << table_entry.has_action() << std::endl;
    // END
    ...
    // BEGIN
    std::cout << "DeviceMgr::table_insert -> p4info ID and table ID do match" << std::endl;
    // END
    RETURN_IF_ERROR(construct_match_key(table_entry, &match_key));
    // BEGIN
    std::cout << "DeviceMgr::table_insert -> after constructing match key" << std::endl;
    // END

    RETURN_IF_ERROR(validate_action(table_entry));
    // BEGIN
    std::cout << "DeviceMgr::table_insert -> after validating action" << std::endl;
    // END
    ...
    // BEGIN
    std::cout << "DeviceMgr::table_insert -> table_entry.action().type_case() = "
      << table_entry.action().type_case() << std::endl;
    // END
    if (table_entry.action().type_case() ==
        p4v1::TableAction::kActionProfileActionSet) {
      // BEGIN
      std::cout << "DeviceMgr::table_insert -> action is of type kActionProfileActionSet (1)" << std::endl;
      // END
      ...
      // BEGIN
      std::cout << "DeviceMgr::table_insert -> action is NOT of type kActionProfileActionSet (1)" << std::endl;
      // END
    ...
    // BEGIN
    std::cout << "DeviceMgr::table_insert -> properly constructed direct resources" << std::endl;
    // END
    ...
    if (table_info_store.get_entry(table_id, match_key) != nullptr) {
      // BEGIN
      std::cout << "DeviceMgr::table_insert -> match already exists" << std::endl;
      // END
      RETURN_ERROR_STATUS(
          Code::ALREADY_EXISTS,
    ...
    // BEGIN
    std::cout << "DeviceMgr::table_insert -> OK after adding content to match table" << std::endl;
    // END
    if (pi_status != PI_STATUS_SUCCESS) {
      RETURN_ERROR_STATUS(Code::UNKNOWN,
    ...
    if (table_entry.action().type_case() ==
        p4v1::TableAction::kActionProfileActionSet) {
      // BEGIN
      std::cout << "DeviceMgr::table_insert -> action is of type kActionProfileActionSet (2)" << std::endl;
      // END
      table_info_store.add_entry(
          table_id, match_key,
      ...
      // BEGIN
      std::cout << "DeviceMgr::table_insert -> action is NOT of type kActionProfileActionSet (2)" << std::endl;
      std::cout << "DeviceMgr::table_insert -> about to add entry to table_info_store" << std::endl;
      // END
      table_info_store.add_entry(
          table_id, match_key,
          TableInfoStore::Data(handle, table_entry.controller_metadata(),
                               table_entry.idle_timeout_ns()));
      // BEGIN
      std::cout << "DeviceMgr::table_insert -> successfully added entry to table_info_store" << std::endl;
      // END
    }
    ...
    // BEGIN
    std::cout << "DeviceMgr::table_insert -> about to return OK status" << std::endl;
    // END
    RETURN_OK_STATUS();
  }
...
DeviceMgr::write(const p4v1::WriteRequest &request) {
  // BEGIN
  std::cout << "DeviceMgr::write -> before pimp::write" << std::endl;
  // END
