...
::util::Status Bmv2Switch::WriteForwardingEntries(
  ...
  // BEGIN
  std::cout << "WriteForwardingEntries -> req.device_id() = " << req.device_id() << std::endl;
  // END
  CHECK_RETURN_IF_FALSE(req.device_id()) << "No device_id in WriteRequest.";
  // BEGIN
  std::cout << "WriteForwardingEntries -> results size = " << results->size() << std::endl;
  // END
  CHECK_RETURN_IF_FALSE(results != nullptr)
      << "Need to provide non-null results pointer for non-empty updates.";

  // BEGIN
  std::cout << "WriteForwardingEntries -> before pi_node" << std::endl;
  // END
